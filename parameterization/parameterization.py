...
while i <= lb+lm:
    ...
    # using law of cosines to trim circle
    d = rs.Distance((i,0,0), keel_pt)
    theta = math.acos(1- (d*d)/(2*R_k*R_k))
    s = R_k*theta
    rs.TrimCurve(ref_circle, [0,s])
    ...
    rs.TrimCurve(l1, [domain1[1],domain1[1]-d1])
    rs.TrimCurve(l2, [domain2[1], domain2[1]-d2])
